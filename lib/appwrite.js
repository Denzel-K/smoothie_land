...
export async function getCurrentUser() {
  try {
    // Get the current authenticated user from Appwrite
    ...
    // Fetch the user document that matches the accountId of the current authenticated user
    ...
    // Return the user document corresponding to the logged-in user
...
export async function getUserTransactions(userId){
  try {
    const transactions = await databases.listDocuments(
      databaseId, orderCollectionId, [Query.equal('creator', userId)]
    )

    if (!transactions) throw new Error("Something went wrong");

    // Parse items from JSON strings to objects
    const transactionsWithParsedItems = transactions.documents.map(transaction => ({
      ...transaction,
      items: transaction.items.map(item => JSON.parse(item))
    }));

    return transactionsWithParsedItems;
  } 
  catch (error) {
    throw new Error(error);
  }
